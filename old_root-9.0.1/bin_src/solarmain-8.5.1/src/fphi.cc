...
static  double calculate_fphi_loglik(double variance, Eigen::VectorXd Sigma,  size_t  n_subjects){
    return -0.5*(log(abs(variance))*n_subjects +  log(abs(Sigma.array())).sum() + n_subjects);
}
...
static Eigen::VectorXd compute_Score(double SD, Eigen::VectorXd residual,Eigen::VectorXd one_minus_lambda, Eigen::MatrixXd SX, Eigen::VectorXd omega_diagonal){
    ...
    Eigen::VectorXd beta_score = SX_transpose*residual_omega;
...
static Eigen::MatrixXd compute_observed_Hessian(double SD, Eigen::VectorXd residual, Eigen::VectorXd one_minus_lambda, Eigen::MatrixXd SX,  Eigen::VectorXd Sigma_inverse){
    ...
    Eigen::VectorXd beta_var_comp_hessian = pow(SD, 2.0)*SX_transpose*(Sigma_inverse.cwiseAbs2()).cwiseProduct(one_minus_lambda.cwiseProduct(residual));
...
static Eigen::ArrayXXd find_max_loglik_2(const int precision, Eigen::VectorXd Y, Eigen::MatrixXd aux, Eigen::MatrixXd X, double & result_loglik, double & result_variance){
    ...
    Eigen::MatrixXd XTOX  = X_transpose*Omega*X;
    Eigen::ArrayXXd parameters;
    if(XTOX.determinant() == 0){
    	return parameters;
    }
    Eigen::VectorXd beta = XTOX.inverse()*X_transpose*Omega*Y;
	...
	XTOX = X_transpose*Omega*X;
   	 if(XTOX.determinant() == 0){
    		return parameters;
   	 }	
	beta = XTOX.inverse()*X_transpose*Omega*Y;
	residual = Y - X*beta;
    ...
    if((h2r >= .9 || h2r <= 0.1) && h2r == h2r){
    	...
    	Eigen::MatrixXd test_XTOX = X_transpose*test_omega*X;
     	if(test_XTOX.determinant() != 0){  	
    		Eigen::VectorXd test_beta = test_XTOX.inverse()*X_transpose*test_omega*Y;
...
static int load_fphi_matrices(Tcl_Interp * interp, Eigen::VectorXd  & trait_vector, Eigen::MatrixXd  & covariate_matrix, Eigen::MatrixXd & auxiliary_matrix, Eigen::MatrixXd & eigenvectors,\
   ...
   // ids_out << fphi_ids[0];
    ...
    for(int row = 0; row <covariate_matrix.rows(); row++){
        if (covariate_matrix(row, 0) == 3) {
            n_missing++;
         }
         
    }
    
    Eigen::VectorXd new_Y(covariate_matrix.rows() - n_missing);
    Eigen::MatrixXd new_covs = Eigen::MatrixXd::Ones(covariate_matrix.rows() - n_missing, 2);
    Eigen::MatrixXd new_covs_two = Eigen::MatrixXd::Ones(covariate_matrix.rows() - n_missing, 3); 
    int index = 0;
    int count_hetero = 0;
    int count_homo = 0;
    for(int row = 0; row < covariate_matrix.rows(); row++){
         if (covariate_matrix(row, 0) != 3) {
            new_Y(index) = trait_vector(row);
            new_covs(index, 0) = covariate_matrix(row, 0);
            if (covariate_matrix(row, 0) == 2){
                new_covs_two(index, 1) = 0.0;
                new_covs_two(index, 2) = 0.0;
                count_homo++;
            }else  if (covariate_matrix(row, 0) == 1){
                new_covs_two(index, 0) = 0.0;
                new_covs_two(index, 2) = 0.0;
                count_hetero++;
            }else{
                new_covs_two(index, 0) = 0.0;
    ...
    cout << count_hetero << " " << count_homo << endl;    
    Eigen::VectorXd beta_one = (new_covs.transpose()*new_covs).inverse()*new_covs.transpose()*new_Y;
    Eigen::VectorXd new_demeaned_Y = new_Y.array() - new_Y.mean();
      Eigen::VectorXd beta_two = (new_covs_two.transpose()*new_covs_two).inverse()*new_covs_two.transpose()*new_demeaned_Y;  
    cout << new_Y(0) - new_covs(0, 0)*beta_one(0) - new_covs(0, 1)*beta_one(1) << endl; 
    cout << new_demeaned_Y(0) - new_covs_two(0, 0)*beta_two(0) - new_covs_two(0, 1)*beta_two(1) - new_covs_two(0, 2)*beta_two(2) << endl;*/// - new_covs_two(0, 3)*beta_two(3)<< endl; 
    saved_eigenvectors = eigenvectors;
    saved_eigenvalues = auxiliary_matrix.col(1);
    saved_ids = loader->get_ids();
    saved_pedigree_filename = pedigree_filename;
...
static void calculate_h2r(Eigen::VectorXd residual, Eigen::MatrixXd Z,  double & h2r , double & loglik, double & SE){
    ...
    Eigen::MatrixXd Z_transpose = Z.transpose();
    Eigen::VectorXd theta = (Z_transpose*Z).inverse()*Z_transpose*F;//Z.colPivHouseholderQr().solve(F);
    if((theta(0) != theta(0) || theta(0) <= 0.0) && (theta(1) <= 0.0 || theta(1) != theta(1))){
        h2r = 0.0;
        SE = 0.0;
        loglik = 0.0;
        return;
    }else if((theta(0) != theta(0) || theta(0) <= 0.0) && theta(1) > 0.0){
        Eigen::VectorXd Z_col_one = Z.col(1);
        double theta_1 =  Z_col_one.dot(F)/Z_col_one.dot(Z_col_one);
        theta(0) = 0.0;
        theta(1) = theta_1;
    }else if(theta(0) > 0.0 && (theta(1) <= 0.0 || theta(1) != theta(1))){
        Eigen::VectorXd Z_col_zero = Z.col(0);
        double theta_0 =  Z_col_zero.dot(F)/Z_col_zero.dot(Z_col_zero);
        theta(0) = theta_0;
        theta(1) = 0.0;
    }    
    ...
    Eigen::VectorXd weights_squared  = pow(weights.array(), -2).matrix();
    Eigen::MatrixXd weighted_Z(weights.rows(), 2);
    weighted_Z.col(0) = Z.col(0).cwiseProduct(weights_squared);
    weighted_Z.col(1) = Z.col(1).cwiseProduct(weights_squared);
    Eigen::MatrixXd weighted_Z_transpose = weighted_Z.transpose();
    Eigen::VectorXd Sigma = (weighted_Z_transpose*Z).inverse()*weighted_Z_transpose*F;
    if((Sigma(0) != Sigma(0) || Sigma(0) <= 0.0) && (Sigma(1) <= 0.0 || Sigma(1) != Sigma(1))){
        h2r = 0.0;
        SE = 0.0;
        loglik = 0.0;
        return;
    }else if((Sigma(0) != Sigma(0) || Sigma(0) <= 0.0) && Sigma(1) > 0.0){
        Eigen::VectorXd Z_col_one = weighted_Z.col(1);
        
        double theta_1 =  Z_col_one.dot(F)/Z_col_one.dot(Z.col(1));
        Sigma(0) = 0.0;
        Sigma(1) = theta_1;
    }else if(Sigma(0) > 0.0 && (Sigma(1) <= 0.0 || Sigma(1) != Sigma(1))){
        Eigen::VectorXd Z_col_zero = weighted_Z.col(0);
        double theta_0 =  Z_col_zero.dot(F)/Z_col_zero.dot(Z.col(0));
        Sigma(0) = theta_0;
        Sigma(1) = 0.0;
    }    
 ...
 //   cout << "(y- xb) E^-1 (y - xb): " << residual.dot(residual.cwiseQuotient(omega)) << endl;
 //   cout << "ln|E|: " << log(omega.array()).sum()  << endl;
    loglik = -0.5*(log(abs(omega.array())).sum() + residual.dot(residual.cwiseQuotient(omega)));
...
static const char * run_fast_fphi_trait_list(const char * list_filename, const char * phenotype_filename, string mask_filename, const char * base_eigen_data_filename = 0){
	...
	if(mask_volume != 0){
	    ...
	    pvalue_volume = new RicVolumeSet(mask_volume->nx, mask_volume->ny, mask_volume->nz, 1);
		pvalue_volume->NIFTIorientation = mask_volume->NIFTIorientation;
	}
	...
	if(base_eigen_data_filename == 0){
	
	    try{
	        reader = new Solar_Trait_Reader(phenotype_filename, trait_list, empty_vector);
	    }catch(Solar_Trait_Reader_Exception & e){
	        return e.what();
	    }catch(...){
	        return "Unknown error occurred when reading phenotype file and computing eigenvalue decomposition";
	    }
	        
	}else{
	    try{
	        reader = new Solar_Trait_Reader(phenotype_filename, base_eigen_data_filename, trait_list);
	    }catch(Solar_Trait_Reader_Exception & e){
	        return e.what();
	    }catch(...){
	        return "Unknown error occurred when reading phenotype file and eigen data";
	    }
	}	
	if(mask_volume == 0){
		ofstream output_stream("list-fphi.out");
		output_stream << "Trait,h2r,loglik,SE,p-value\n";
		output_stream.close();
	}
	...
	for(unsigned set = 0; set < reader->get_n_sets(); set++){
		...
		vector<string> eigen_trait_list = eigen_data->get_trait_names();
		/*for(int i = 0; i < eigen_trait_list.size(); i++){
		    cout << eigen_trait_list[i] << endl;
		}
	    cout << "end\n"*/
		vector<double> h2r_list(eigen_trait_list.size());
		vector<double> loglik_list(eigen_trait_list.size());
		vector<double> SE_list(eigen_trait_list.size());
		vector<double> pvalue_list(eigen_trait_list.size());
#pragma omp parallel for
		for(unsigned trait = 0; trait < eigen_trait_list.size() ; trait++){
			...
			double h2r, loglik, SE, null_loglik, null_variance;
			null_variance = residual.squaredNorm()/residual.rows();
			null_loglik = -0.5*(residual.rows()*log(null_variance) + residual.rows());
			calculate_h2r(residual, aux_matrix, h2r, loglik,SE);
			h2r_list[trait] = h2r;
			loglik_list[trait] = loglik;
			if(null_loglik < loglik){
			    pvalue_list[trait] = chicdf(2.0*(loglik - null_loglik), 1);
			}else{
			    pvalue_list[trait] = 0.5;
			} 
			SE_list[trait] = SE;
		}
		if(mask_volume == 0){
			...
			for(unsigned trait = 0; trait < eigen_trait_list.size(); trait++){
				output_stream << eigen_trait_list[trait] << "," << h2r_list[trait] << "," << loglik_list[trait] << "," << SE_list[trait] << "," <<  pvalue_list[trait] <<  "\n";
			}
			output_stream.close();
		}else{
			for(unsigned i = 0; i < eigen_trait_list.size(); i++){
				vector<string> indices = convert_volume_indices(eigen_trait_list[i]);
				...
				pvalue_volume->VolSet[0].vox[x][y][z] = pvalue_list[i];
			}
		}
  ...
  chrono::duration<double> time_span = chrono::duration_cast<chrono::duration<double>>(t2 - t1);
	...
	if(mask_volume != 0){
		...
		pvalue_volume->Write("pvalue"+base_filename);
		delete h2r_volume;
		delete loglik_volume;
		delete mask_volume;
		delete se_volume;
		delete pvalue_volume;
	}
...
static int method_of_moments(Tcl_Interp * interp, string trait_name, string phenotype_filename, double & sigma_a, double & sigma_e){
    vector<string> terms_to_be_used;
    terms_to_be_used.push_back(trait_name);
    solar_mle_setup * loader;
    Eigen::MatrixXd output_matrix;
    ...
    Eigen::VectorXd trait_vector = output_matrix.col(0);
    trait_vector = trait_vector.array() - trait_vector.mean();
    Eigen::VectorXd Y(trait_vector.rows()*(trait_vector.rows() + 1)/2);
    Eigen::MatrixXd X(Y.rows(), 2);
    const int n_subjects = trait_vector.rows();
    int index = 0;
    for(int col = 0; col < n_subjects; col++){
        X(index, 0) = 1.0;
        X(index, 1) = phi2_matrix(col, col);
        Y(index) = trait_vector(col)*trait_vector(col);
        ++index;
        for(int row = col + 1; row < n_subjects; row++){
            X(index,0) = 0.0;
            X(index, 1) = phi2_matrix(row,col);
            Y(index) = trait_vector(col)*trait_vector(row);
            ++index;
        }
    }
    ...
    Eigen::MatrixXd X_transpose = X.transpose();
    Eigen::MatrixXd XTX = X_transpose*X;
    Sigma = XTX.inverse()*X_transpose*Y;
 
    if((Sigma(0) <= 0.0 || Sigma(0) != Sigma(0)) && Sigma(1) > 0.0){
        Sigma(0) = 0.0;
        Eigen::VectorXd X_col_one = X.col(1);
        const double X_col_one_squared = X_col_one.dot(X_col_one);
        Sigma(1) = X_col_one.dot(Y)/X_col_one_squared;
        if(Sigma(1) < 0.0 || Sigma(1) != Sigma(1)) Sigma(1) = 0.0;
        sigma_a = Sigma(1);
        sigma_e = Sigma(0);
        ...
        Sigma(1) = 0.0;
        Eigen::VectorXd X_col_zero = X.col(0);
        const double X_col_zero_squared = X_col_zero.dot(X_col_zero);
        Sigma(0) = X_col_zero.dot(Y)/X_col_zero_squared;
        if(Sigma(0) < 0.0 || Sigma(0) != Sigma(0)) Sigma(0) = 0.0;
        sigma_a = Sigma(1);
        sigma_e = Sigma(0);
       
    }else if ((Sigma(1) <= 0.0 || Sigma(1) != Sigma(1)) && (Sigma(0) <= 0.0 || Sigma(0) != Sigma(0))){
        Sigma(0) = 0.0;
        Sigma(1) = 0.0;
        sigma_a = Sigma(1);
        sigma_e = Sigma(0);
...
static void format_fphi_output(double h2r, double se, double pvalue, double loglik, double sporadic_loglik, const char * pedigree_filename, const char * phenotype_filename,\
    ...
    cout <<   "  Trait:   " << trait_name  << " H2r = " << h2r << " SE = " << se  << "  Individuals:  " << n_subjects << endl;
    cout <<   "  polygenic loglik:   " << loglik << "  sporadic loglik:   " << sporadic_loglik <<  "  p = " << pvalue << " " << pvalue_message << endl;
...
extern "C" int runfphiCmd(ClientData clientData, Tcl_Interp * interp,
    ...
    double h = 0.0005;
    double relax = 1.0;
    bool use_method_of_moments = false;
    for(int arg = 1 ;arg < argc ; arg++){
        if(!StringCmp(argv[arg], "help", case_ins) || !StringCmp(argv[arg], "-help", case_ins) || !StringCmp(argv[arg], "--help", case_ins)
           || !StringCmp(argv[arg], "h", case_ins) || !StringCmp(argv[arg], "-h", case_ins) || !StringCmp(argv[arg], "--help", case_ins)){
            ...
            converge = false;
        }else if (!StringCmp(argv[arg], "-mom", case_ins) || !StringCmp(argv[arg], "--mom", case_ins) ){
    ...
    /*const char * pedigree_filename = 0;
    try {
        pedigree_filename = currentPed->filename();
    }catch(...){
        RESULT_LIT("No pedigree has been loaded");
        return TCL_ERROR;
    }
    if (pedigree_filename == 0){
        RESULT_LIT("No pedigree has been loaded");
        return TCL_ERROR;
    }*/
    ...
    if(list_filename){
	    ...
	    error_message = run_fast_fphi_trait_list(list_filename, phenotype_filename, mask_filename, evd_data_filename);
	    if(!error_message){
    ...
    if( Trait::Number_Of() != 1){
        RESULT_LIT("Either no trait has been selected or more than two have been selected");
        return TCL_ERROR;
    }
    ...
    if (use_method_of_moments){
        double sigma_a, sigma_e;
        success = method_of_moments(interp, trait_list[0], phenotype_filename, sigma_a, sigma_e);
        if (success == TCL_OK){
            cout << "FPHI -- Method of Moments Estimate\n";
            cout << "  Genetic Variance: " << sigma_a << " Std Error: " <<  endl;
            cout << "  Residual Variance: " << sigma_e << " Std Error: " <<  endl;
            cout << "  Total Variance: " << sigma_a + sigma_e << endl;
            if (sigma_a + sigma_e == 0.0){
                cout << "  H2r: 0.0 Std Error: 0.0\n";
            }else{
                
                cout << "  H2r: " << sigma_a/(sigma_a + sigma_e) << endl;
            }
            return success;
        }else{
            return TCL_ERROR;
    ...
    if(converge){
        ...
        Eigen::ArrayXXd parameters =find_max_loglik_2(11, Y, aux, X, loglik,  variance);
	auto finish = std::chrono::high_resolution_clock::now();
        ...
        if(parameters(parameters.rows() - 2, 0) != 0.0){
            ...
            pvalue =  calculate_pvalue(residual, loglik, sporadic_loglik);
        }
        cout.precision(12);
        format_fphi_output ( parameters(cov_matrix.cols() + 1, 0), parameters(cov_matrix.cols() + 1, 1),  pvalue, loglik, sporadic_loglik,  pedigree_filename,  phenotype_filename,\
      ...
      //  cout << dur.count() << " microseconds\n";
        ...
        Eigen::MatrixXd eigenvectors_transpose = eigenvectors.transpose();
        Eigen::VectorXd Y = eigenvectors_transpose*(trait_v.array() - trait_v.mean()).matrix();
        Eigen::VectorXd residual;
        if(cov_matrix.cols() > 1) {
            Eigen::MatrixXd X = eigenvectors_transpose*cov_matrix;
            Eigen::MatrixXd X_transpose = X.transpose();
            Eigen::MatrixXd hat_matrix = Eigen::MatrixXd::Identity(X.rows(), X.rows()) - X*(X_transpose*X).inverse()*X_transpose;
        ...
        format_fphi_output ( h2r, SE,  pvalue, loglik, sporadic_loglik,  pedigree_filename,  phenotype_filename,\
                            trait_list[0].c_str(), trait_v.rows());
