...
const char * gpu_gwas_screen(vector<int> gpu_ids, vector<string> trait_list, const char*  phenotype_filename, const char* plink_filename, const char * evd_data_filename, int defined_max_batch_size, int defined_thread_size, int n_streams, const bool blank_ped, const bool verbose);
...
extern "C" int  GPU_GWAS_Cmd(ClientData clientData, Tcl_Interp *interp,
    ...
    for(int arg = 1 ; arg < argc; arg++){
		...
		verbose = true;
        }else if (!StringCmp(argv[arg], "--blank_ped", case_ins) || !StringCmp(argv[arg], "-blank_ped", case_ins)){
        blank_ped = true;
    ...
    if(!loadedPed() && evd_data_filename == 0 && !blank_ped){
    ...
    if(!use_screen && blank_ped){
        RESULT_LIT("-blank_ped option must be used with the -screen option");
        return TCL_ERROR;
    }
    ...
    if(!evd_data_filename && !blank_ped){
    	try {
    ...
    if(use_screen){
    	const char * errmsg = 0;
    	try{
		errmsg = gpu_gwas_screen(gpu_id_list, trait_list, phenotype_filename, plink_filename,  evd_data_filename,  defined_batch_size,  blockSize,  stream_count, blank_ped, verbose);
